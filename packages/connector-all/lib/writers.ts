...
import type { IBrokerConfig } from '@treecg/connector-kafka/lib/Common';
...
import type { IFileWriterConfig,
  WriterConfig,
  IWsWriterConfig,
  IKafkaWriterConfig,
  IHttpWriterConfig,
  MatchFunctionObject } from './connectorAll';
import { getOne, parseBool } from './util';
import { CONN } from './voc';

export async function loadWriterConfig(subject: rdf.Term, match: MatchFunction): Promise<ITyped<WriterConfig>> {
...
async function objToKafkaConfig(subj: rdf.Term, match: MatchFunctionObject): Promise<IKafkaWriterConfig> {
  const out = <IKafkaWriterConfig>{};
  // Types? What types
...
async function objToHTTPConfig(subj: rdf.Term, match: MatchFunctionObject): Promise<IHttpWriterConfig> {
...
async function objToFileConfig(subj: rdf.Term, match: MatchFunctionObject): Promise<IFileWriterConfig> {
...
async function objToWsConfig(subj: rdf.Term, match: MatchFunctionObject): Promise<IWsWriterConfig> {
